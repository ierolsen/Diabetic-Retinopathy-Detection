...
When we check the images, we can realize that we should apply some operations before training. These operations will be used for destroying black area, making clear blood vessels.
...
Above, I read first 20 images, because totaly there are 3662 in training dataset, I just want to take a look as first step.

As is seen, the second image has huge black area, we don't want it, cause it avoids learning and also it makes images smaller. Therefore I will crop these black areas.

**BUT for now, I will go on just first image, after complete these steps in first image, I will define a function that makes apply these steps to all datasets.**


To crop the image, first I need to convert it to **GRAYSCALE** because I will apply **THRESHOLD** and in order to apply that, image should be **GRAY**.

![im3](https://user-images.githubusercontent.com/30235603/104626104-c9c24500-5695-11eb-9826-1e891d6d4e86.png)

Before **THRESHOLD**, I need to apply **Median Blurring** becuase my aim is destroying black area and applying Blurring operation, it makes contour of image clear.
...
To find **Contour** I will use a method call **cv2.findContours()** When we use this method, it return an array
...
After get the list of coordinates, I will find max and min values for X and Y axises. 

![im8](https://user-images.githubusercontent.com/30235603/104626111-caf37200-5695-11eb-9c7e-fb7c4d904631.png)


Now I have a coordinates for cropping operations. So I can crop it. To crop image there is no method actually, I just use slicing.

![im9](https://user-images.githubusercontent.com/30235603/104626112-caf37200-5695-11eb-9a5f-f57889930b48.png)


The image has been cropped! **BUT** as you can see, its size are changed due to crop. So now, I need to **resize** it again. 
...
Now We are in important part called **CLAHE**
...
In order to apply **CLAHE** image should be **LAB** format, for that, I will convert image according to this method **cv2.COLOR_RGB2LAB**
...
Above the image, it has 2 size now.
...
now image has a more **Symmetrical Distribution**.
...
Now that I have applied **CLAHE** I can move on other steps,

The other step is **MERGING**, before I splited L,A,B channels, now it's time to **MERGE** them. In order to merge them, I will use **cv2.merge()**

![im16](https://user-images.githubusercontent.com/30235603/104626121-cd55cc00-5695-11eb-8cb2-2a6f5f9ccf85.png)

Now I have a really good looking image but actually I shouldn't say good looking because it has a noises (Salt-Peper). The best way to destroy them is using *BLURRING OPERATION**
...
The other important, even most important step is making blood vessels more clear.

To do that, **First** I will apply **BLURRING** with a huge kernel size, because it makes image more blur, just the blood vessels will be appeared, after that I will have a backgorung that has appeared blood vassels.
...
also beta=-1 it means, reverse all values of pixels. Gamma=255 it is for reverse image color. as you can see it turned white. And thus, only blood vessels appears.


Now I will apply to mask to image. For that I will use **cv2.bitwise_and()** We can say that it merges two images.
...
But here there is a really important situation. Images are not **Multi Class** they are **Multi Label** In same image there are more probabilities. I mean if the patient's scala is 3 (it means "Severe") also patient has 0,1 and 2 scalas. Because these symptoms correlated with eachself. 
**Summary* If patient has a scala 2 problem, we can see scala 0, scala 1 and scala 2, as told before they are correlated with eachself. 
...
And now I am ready to define transfer learning model but first I need to transform my images to arrays. It is so simple with this method **np.array()**

![im25](https://user-images.githubusercontent.com/30235603/104626092-c6c75480-5695-11eb-888e-835bd7b9f963.png)

So, one more step to define model, it's time to split data:
...
Now it's time to define transfer learning model. But first, the model must be downloaded:
...
In transfer learning model called **EfficientNetB5** I chose include_top=False because I don't want to use last part of layers, also I've changed the input size as (240,240,3) first it was (400,400,3) but in my pc it doesn't work because of the RAM :) 
...
**Unfortunately** my pc doesn't have enough RAM so because of that I can not train my model :( but I will figure out that.
